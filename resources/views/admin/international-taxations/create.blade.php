...
@section('title', 'Add International Taxation - Admin Panel')

@section('content')
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center;">
    <h1 class="page-title">🌍 Add International Taxation</h1>
    <a href="{{ route('admin.international-taxations.index') }}" class="btn btn-secondary">← Back to International Taxation</a>
</div>

<div class="card">
    <form action="{{ route('admin.international-taxations.store') }}" method="POST">
        @csrf
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 15px;">
                <div>
                    ...
                    >
                        ...
                        <option value="Tax Treaties" {{ old('category') == 'Tax Treaties' ? 'selected' : '' }}>Tax Treaties</option>
                        <option value="Treaty Comparison" {{ old('category') == 'Treaty Comparison' ? 'selected' : '' }}>Treaty Comparison</option>
                        <option value="Transfer Pricing" {{ old('category') == 'Transfer Pricing' ? 'selected' : '' }}>Transfer Pricing</option>
                        <option value="Double Taxation" {{ old('category') == 'Double Taxation' ? 'selected' : '' }}>Double Taxation</option>
                        <option value="Tax Information Exchange" {{ old('category') == 'Tax Information Exchange' ? 'selected' : '' }}>Tax Information Exchange</option>
        ...
        <div style="display: flex; gap: 10px; justify-content: flex-end; border-top: 1px solid #e5e7eb; padding-top: 20px;">
            <button type="submit" class="btn btn-primary">Create International Taxation</button>
