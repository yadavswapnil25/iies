...
<div class="page-header" style="display:flex;justify-content:space-between;align-items:center;">
  <h1 class="page-title">🔔 Add Order / Notice / Notification</h1>
  <a href="{{ route('admin.order-notices.index') }}" class="btn btn-secondary">← Back</a>
</div>

<div class="card">
  <form method="post" action="{{ route('admin.order-notices.store') }}">
    @csrf
    ...
    <div style="margin-bottom:20px;">
      ...
      <div style="margin-bottom:15px;">
        ...
        <input type="text" id="title" name="title" value="{{ old('title') }}" placeholder="Enter title" required style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:6px;@error('title') border-color:#dc2626; @enderror">
      ...
      <div style="margin-bottom:15px;">
        ...
        <textarea id="content" name="content" rows="8" required style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:6px;@error('content') border-color:#dc2626; @enderror" placeholder="Write the order/notice text here...">{{ old('content') }}</textarea>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:15px;">
        <div>
          ...
          <input type="datetime-local" id="published_at" name="published_at" class="form-input" value="{{ old('published_at') }}" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:6px;">
          <small style="color:#6b7280;font-size:12px;">Optional; defaults to created time</small>
        </div>
        <div>
          <label style="display:block;margin-bottom:5px;font-weight:600;color:#374151;">Status</label>
          <div style="display:flex;align-items:center;gap:8px;">
            <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} style="margin:0;">
