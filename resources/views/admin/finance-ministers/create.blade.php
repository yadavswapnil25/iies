...
<div class="content-header">
    ...
    <h1>Add New Finance Minister</h1>
    <p>Register a new Finance Minister or Minister of State</p>
</div>

<div class="content-body">
    <form method="POST" action="{{ route('admin.finance-ministers.store') }}" class="minister-form" enctype="multipart/form-data">
        @csrf
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select id="designation" name="designation" class="form-select @error('designation') error @enderror" required>
                        ...
                        <option value="Finance Minister" {{ old('designation') == 'Finance Minister' ? 'selected' : '' }}>Finance Minister</option>
                        <option value="Minister of State" {{ old('designation') == 'Minister of State' ? 'selected' : '' }}>Minister of State</option>
                    </select>
                    @error('designation')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
        ...
        <div class="form-section">
            <h2>Image & Bio</h2>
            <div class="form-group">
                <label for="image">Minister Photo</label>
                <input type="file" id="image" name="image" class="form-input @error('image') error @enderror" 
                       accept="image/*">
                @error('image')
                    <span class="error-message">{{ $message }}</span>
                @enderror
                <small class="form-help">Upload a professional photo (max 2MB, JPG/PNG/GIF)</small>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <textarea id="bio" name="bio" class="form-textarea @error('bio') error @enderror" 
                              rows="4" placeholder="Brief biography of the minister...">{{ old('bio') }}</textarea>
                    @error('bio')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <textarea id="hindi_bio" name="hindi_bio" class="form-textarea @error('hindi_bio') error @enderror" 
                              rows="4" placeholder="मंत्री का संक्षिप्त जीवन परिचय...">{{ old('hindi_bio') }}</textarea>
                    @error('hindi_bio')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-primary">Create Minister</button>
